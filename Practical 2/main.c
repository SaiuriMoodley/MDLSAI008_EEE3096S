...
static uint16_t addresses[] = {0b0000000000000000, 0b0000000000001000, 0b0000000000010000, 0b0000000000011000,
							   0b0000000000100000, 0b0000000000101000};
static int index = 0; //tracks next address input for read function
static bool delayFlag = false; //used to toggle the delay (1s OR 0.5s)
...
int main(void)
{
  ...
  for(int i = 0; i < 6; i++) {
	  write_to_address(addresses[i], patterns[i]);
  }
  ...
  while (1)
  {
	...
	// TODO: Check button PA0; if pressed, change timer delay
	  ...
	  if ((GPIOA->IDR & Button0_Pin) == 0) { //PA0 pressed
		  delayFlag = !delayFlag;
		  if (delayFlag) {
		  ...
		  HAL_TIM_Base_Init(&htim16); // initialize TIM16 with new ARR value
		  HAL_TIM_Base_Start_IT(&htim16); //start TIM16
	  }
...
void TIM16_IRQHandler(void)
{
	...
	uint8_t pattern = read_from_address(addresses[index]);
	if (pattern == patterns[index])
		GPIOB -> ODR = pattern;
	else GPIOB -> ODR = 0b00000001; //SPI failure
	index++;
	index = index % 6; //wrap around to 0 when index = 6
}
