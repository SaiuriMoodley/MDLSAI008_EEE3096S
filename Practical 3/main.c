...
int main(void)
{
  ...
  while (1)
  {
	...
	uint32_t val = pollADC(); //get converted value
	char buffer[100];
	sprintf(buffer, "%lu", val); //store formatted (specifier = unsigned long integer) string
	writeLCD(buffer);
...
void EXTI0_1_IRQHandler(void)
{
	...
	if ((curr_millis - prev_millis) > 200) { //delay preventing debouncing
		if (delay_t == 500) { //toggle LED frequency
			delay_t = 1000;
		} else {
			delay_t = 500;
		}
...
void writeLCD(char *char_in){
    delay(3000);
	lcd_command(CLEAR);
	lcd_putstring(char_in); //write string to screen
}
...
uint32_t pollADC(void){
  // TODO: Complete function body to get ADC val
	...
	HAL_ADC_PollForConversion(&hadc, HAL_MAX_DELAY); //wait for end of conversion
	uint32_t val = HAL_ADC_GetValue(&hadc); //read converted value
...
uint32_t ADCtoCCR(uint32_t adc_val){
  // TODO: Calculate CCR val using an appropriate equation
	uint32_t val = adc_val / 4095 * 47999 ; //CCR = ADC reading / Max ADC reading * ARR
	return val;
}
